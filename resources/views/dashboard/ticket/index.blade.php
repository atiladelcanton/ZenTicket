            ...
            <div class="table-responsive">
                <table class="table header-border table-hover table-custom spacing5">
                    ...
                    @foreach($tickets as $key => $ticket)
                        ...
                        <tr>
                            <td>
                                <a href="{{route('chamados.detail',$ticket->ticket_number)}}">{{$ticket->ticket_number}}</a>
                            </td>
                            <td>{{$ticket->project->name}}</td>
                            <td>{{$ticket->title}}</td>
                            <td><span class="badge"
                                      style="background: transparent; color:{{$ticket->priority->color}}; border: 1px solid {{$ticket->priority->color}};">{{$ticket->priority->name}}</span>
                            </td>
                            <td><span class="badge badge-danger"
                                      style="background: transparent; color:{{$ticket->type->color}}; border: 1px solid {{$ticket->type->color}};">{{$ticket->type->name}}</span>
                            </td>
                            <td><span class="badge badge-default"
                                      style="background: transparent; color:{{$ticket->impact->color}}; border: 1px solid {{$ticket->impact->color}};">{{$ticket->impact->name}}</span>
                            </td>

                            <td>{{$ticket->responsible_ticket? $ticket->userResponsible->name : 'Aguardando...' }}</td>
